...
class Camera:
    ...
    def quat_from_ypr(self, yaw=0, pitch=0, roll=0):
        ...
        qy = np.cos(roll/2) * np.sin(pitch/2) * np.cos(yaw /
                                                                 2) + np.sin(roll/2) * np.cos(pitch/2) * np.sin(yaw/2)
        qz = np.cos(roll/2) * np.cos(pitch/2) * np.sin(yaw /
                                                                 2) - np.sin(roll/2) * np.sin(pitch/2) * np.cos(yaw/2)
    ...
    def generate_intrinsic_matrix(self, focal_length=1, skew_factor=0, offset_x=0.5, offset_y=0.5, aspect_ratio=1):
        ''' Create an intrinsic matrix '''
        return np.array([[focal_length, skew_factor, offset_x],
