...
class Renderer:
    ...
    def world2image(self, world_points, camera):
        ...
        image_points, visible_indices = self.project_points(camera_points, camera, (h, w))
    ...
    def filter_image_points(self, camera, image_points, visible_indices):
        ...
        #print(f'inframe: {len(image_points)}')
    ...
    def cube2surface(self, points, dim):
        ...
        surface_corner_offsets = np.insert(
            other_dims_offsets, dim, 0.5, axis=1)
    ...
    def fill_surfaces(self, world_quads, image_quads, quad_values, min_area=1):
        ...
        for world_quad, image_quad, image_area, value in zip(world_quads, image_quads, image_areas, quad_values):
            ...
            dim_a = np.linspace(np.min(world_quad[:, nonfixed_dims[0]]), np.max(
                world_quad[:, nonfixed_dims[0]]), side)
            dim_b = np.linspace(np.min(world_quad[:, nonfixed_dims[1]]), np.max(
                world_quad[:, nonfixed_dims[1]]), side)
    ...
    def render(self, world_array, camera):
        ...
        x_surface_pos_ip, x_surface_pos_indices = self.filter_image_points(camera, x_surface_pos_ip, x_surface_pos_indices)
        x_surface_neg_ip, x_surface_neg_indices = self.filter_image_points(camera, x_surface_neg_ip, x_surface_neg_indices)
        y_surface_pos_ip, y_surface_pos_indices = self.filter_image_points(camera, y_surface_pos_ip, y_surface_pos_indices)
        y_surface_neg_ip, y_surface_neg_indices = self.filter_image_points(camera, y_surface_neg_ip, y_surface_neg_indices)
